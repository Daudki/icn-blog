...
class ICNBuilder:
    ...
    def _flatten_modules(self):
        """Convert nested JSON structure to flat list of modules with metadata."""
        flat = []
    ...
    def _build_index(self):
        ...
        # Module cards HTML (all modules, rendered by JS)
        # We embed the module data as JSON so JS can render it
...
<body>
    ...
    <main>
        <section class="hero">
            ...
            <div class="hero-btns">
                ...
                <a href="#modules" class="btn btn-outline"><i class="fa-solid fa-magnifying-glass"></i> Search</a>
            </div>
        </section>
    ...
    <footer class="footer">
        <p>ICN Blog — Built by <strong>theOutcast</strong> | Mbeya University of Science & Technology</p>
        <p style="margin-top:0.3rem;">Python × Networking × Security</p>
    </footer>

    <script>
        // All module data embedded directly (no fetch needed — works offline)
        ...
        function getYearClass(mod) {{ return mod.year_key === 'year1' ? 'y1' : mod.year_key === 'year2' ? 'y2' : mod.year_key === 'year3' ? 'y3' : 'y4'; }}
        ...
        function renderModules() {{
            ...
            grid.innerHTML = filtered.map(m => {{
                ...
                return `
                <div class="mod-card ${{getYearClass(m)}}" onclick="window.location.href='${{m.url}}'" tabindex="0" onkeydown="if(event.key==='Enter')window.location.href='${{m.url}}'">
                    <div class="icon-row">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>About | ICN Blog</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <style>
        :root { --bg: #0a0e17; --bg-card: #111827; --border: #21262d; --text: #e6edf3; --text-muted: #8b949e; --accent: #58a6ff; --radius: 8px; --max: 800px; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', system-ui, sans-serif; background: var(--bg); color: var(--text); line-height: 1.7; min-height: 100vh; }
        .header { background: var(--bg-card); border-bottom: 1px solid var(--border); padding: 0 1.25rem; height: 60px; display: flex; align-items: center; }
        .header a { color: var(--text); text-decoration: none; font-weight: 700; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem; }
        .header a i { color: var(--accent); }
        ...
        .footer { text-align: center; padding: 1.5rem; border-top: 1px solid var(--border); color: var(--text-muted); font-size: 0.85rem; margin-top: 3rem; }
    </style>
</head>
<body>
    <header class="header"><a href="index.html"><i class="fa-solid fa-network-wired"></i> ICN Blog</a></header>
    <div class="container">
        <h1>About ICN Blog</h1>
        <div class="card">
            <h3><i class="fa-solid fa-bullseye"></i> Purpose</h3>
            <p>This blog transforms the BSc Information & Computer Networking curriculum at Mbeya University of Science and Technology into practical, code-driven Python projects. Every module gets at least one working project that bridges theory and practice.</p>
        </div>
        <div class="card">
            <h3><i class="fa-solid fa-user"></i> Author</h3>
            <p>Built by <strong>theOutcast</strong> — an ICN student who believes learning happens when you build, not just when you read.</p>
        </div>
        <div class="card">
            <h3><i class="fa-solid fa-code"></i> Philosophy</h3>
            <p>Code-first learning. Detailed line-by-line explanations. Offline-ready. Every project includes breakdowns so you understand what every line does — not just copy-paste.</p>
        </div>
        <div class="card">
            <h3><i class="fa-solid fa-python"></i> Technologies</h3>
            <p>Python, Scapy, Netmiko, Scikit-learn, Flask, SQLite, Docker, and more — all applied to networking, security, and automation.</p>
        </div>
    </div>
    <footer class="footer"><p>ICN Blog — Built by <strong>theOutcast</strong> | MUST</p></footer>
</body>
</html>'''
        (self.build_dir / 'about.html').write_text(html, encoding='utf-8')
            ...
            projects_html += f'''
                <div class="project-card">
                    <div class="project-header" onclick="this.parentElement.querySelector('.project-body').classList.toggle('open'); this.classList.toggle('open');">
                        ...
                        <span class="difficulty {proj.get('difficulty', 'Beginner').lower()}">{proj.get('difficulty', 'Beginner')}</span>
                    </div>
                    <div class="project-body{' open' if i == 0 else ''}">
                        ...
                        <div class="code-block">
                            <div class="code-header">
                                ...
                                <button class="copy-btn" onclick="copyCode(this, '{code_id}')"><i class="fa-solid fa-copy"></i> Copy</button>
                            </div>
...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <style>
        :root {{ --bg: #0a0e17; --bg-card: #111827; --bg-code: #0d1117; --border: #21262d; --text: #e6edf3; --text-muted: #8b949e; --accent: #58a6ff; --green: #3fb950; --orange: #d2991d; --red: #f85149; --purple: #a371f7; --radius: 8px; --max: 1000px; }}
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: 'Segoe UI', system-ui, sans-serif; background: var(--bg); color: var(--text); line-height: 1.7; }}
        a {{ color: var(--accent); text-decoration: none; }}
        .header {{ background: var(--bg-card); border-bottom: 1px solid var(--border); padding: 0 1.25rem; height: 60px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 100; }}
        .header a {{ color: var(--text); font-weight: 700; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem; }}
        .header a i {{ color: var(--accent); }}
        .header .back {{ color: var(--text-muted); font-size: 0.9rem; }}
        .header .back:hover {{ color: var(--text); }}
        .breadcrumb {{ max-width: var(--max); margin: 0 auto; padding: 0.75rem 1.25rem; font-size: 0.85rem; color: var(--text-muted); }}
        .breadcrumb a {{ color: var(--accent); }}
        .hero {{ max-width: var(--max); margin: 0 auto; padding: 2rem 1.25rem; }}
        .hero .badge {{ display: inline-block; background: rgba(88,166,255,0.1); color: var(--accent); padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.8rem; margin-bottom: 0.75rem; }}
        .hero h1 {{ font-size: 2.2rem; margin-bottom: 0.3rem; }}
        .hero .code {{ font-family: monospace; color: var(--accent); font-size: 1rem; margin-bottom: 0.75rem; }}
        .hero .desc {{ color: var(--text-muted); font-size: 1.05rem; margin-bottom: 1rem; }}
        .hero .meta {{ display: flex; gap: 1rem; flex-wrap: wrap; font-size: 0.85rem; color: var(--text-muted); margin-bottom: 0.75rem; }}
        .hero .tags {{ display: flex; gap: 0.4rem; flex-wrap: wrap; }}
        .hero .tags span {{ background: var(--bg-card); border: 1px solid var(--border); padding: 0.2rem 0.6rem; border-radius: 4px; font-size: 0.75rem; color: var(--text-muted); font-family: monospace; }}
        .content {{ max-width: var(--max); margin: 0 auto; padding: 0 1.25rem 3rem; }}
        .content h2 {{ font-size: 1.4rem; margin: 2rem 0 1rem; display: flex; align-items: center; gap: 0.5rem; }}
        .content h2 i {{ color: var(--accent); }}
        .objectives {{ list-style: none; display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; margin-bottom: 1.5rem; }}
        .objectives li {{ display: flex; align-items: center; gap: 0.5rem; color: var(--text-muted); font-size: 0.9rem; }}
        .objectives li i {{ color: var(--green); }}
        .project-card {{ background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); margin-bottom: 1.25rem; overflow: hidden; }}
        .project-header {{ display: flex; justify-content: space-between; align-items: center; padding: 1.1rem 1.25rem; cursor: pointer; border-bottom: 1px solid var(--border); }}
        .project-header:hover {{ background: rgba(255,255,255,0.02); }}
        .project-header h3 {{ font-size: 1.05rem; display: flex; align-items: center; gap: 0.5rem; }}
        .project-header h3 i {{ color: var(--green); }}
        .difficulty {{ font-size: 0.7rem; padding: 0.2rem 0.6rem; border-radius: 12px; font-weight: 600; }}
        .difficulty.beginner {{ background: rgba(63,185,80,0.1); color: var(--green); }}
        .difficulty.intermediate {{ background: rgba(210,153,29,0.1); color: var(--orange); }}
        .difficulty.advanced {{ background: rgba(248,81,73,0.1); color: var(--red); }}
        .project-body {{ display: none; padding: 1.25rem; }}
        .project-body.open {{ display: block; }}
        .project-body > p {{ color: var(--text-muted); margin-bottom: 1rem; }}
        .code-block {{ background: var(--bg-code); border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; margin: 1rem 0; }}
        .code-header {{ display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 1rem; background: #161b22; border-bottom: 1px solid var(--border); }}
        .code-header span {{ font-family: monospace; font-size: 0.8rem; color: var(--text-muted); }}
        .copy-btn {{ background: #21262d; border: 1px solid #30363d; color: var(--text-muted); padding: 0.3rem 0.7rem; border-radius: 4px; cursor: pointer; font-size: 0.75rem; transition: all 0.15s; }}
        .copy-btn:hover {{ background: #30363d; color: var(--text); }}
        .copy-btn.copied {{ background: rgba(63,185,80,0.2); border-color: var(--green); color: var(--green); }}
        .code-block pre {{ padding: 1rem 1.25rem; margin: 0; overflow-x: auto; }}
        .code-block code {{ font-family: 'JetBrains Mono', 'Fira Code', 'Consolas', monospace; font-size: 0.8rem; line-height: 1.6; color: #c9d1d9; }}
        .explanation {{ background: rgba(88,166,255,0.04); border-left: 3px solid var(--accent); padding: 1rem 1.25rem; border-radius: 0 var(--radius) var(--radius) 0; margin: 1rem 0; }}
        .explanation h4 {{ color: var(--accent); margin-bottom: 0.5rem; font-size: 0.9rem; }}
        .explanation ol {{ padding-left: 1.25rem; }}
        .explanation li {{ color: var(--text-muted); margin-bottom: 0.5rem; font-size: 0.85rem; line-height: 1.6; }}
        .explanation li strong {{ color: var(--text); }}
        .explanation code {{ background: rgba(88,166,255,0.1); padding: 0.1rem 0.35rem; border-radius: 3px; font-size: 0.85em; }}
        .footer {{ background: var(--bg-card); border-top: 1px solid var(--border); text-align: center; padding: 1.5rem; margin-top: 3rem; color: var(--text-muted); font-size: 0.85rem; }}
        @media (max-width: 768px) {{ .hero h1 {{ font-size: 1.5rem; }} .objectives {{ grid-template-columns: 1fr; }} }}
    </style>
</head>
<body>
    <header class="header">
        <a href="../../index.html"><i class="fa-solid fa-network-wired"></i> ICN Blog</a>
        <a href="../../index.html#modules" class="back"><i class="fa-solid fa-arrow-left"></i> All Modules</a>
    </header>
    <nav class="breadcrumb">
        <a href="../../index.html">Home</a> / <a href="../../index.html#modules">Modules</a> / <span>{mod['name']}</span>
    </nav>
    <section class="hero">
        <span class="badge">{mod['year_label']} · {mod['sem_label']}</span>
        <h1>{mod['name']}</h1>
        <p class="code">{mod.get('code', '')}</p>
        <p class="desc">{mod.get('description', '')}</p>
        <div class="meta">
        ...
        <div class="tags">{''.join(f'<span>{tag}</span>' for tag in mod.get('tags', []))}</div>
    </section>
    <main class="content">
    ...
    <footer class="footer"><p>ICN Blog — Built by <strong>theOutcast</strong> | MUST</p><p style="margin-top:0.3rem;">Python × Networking × Security</p></footer>
    <script>
        function copyCode(btn, codeId) {{
            ...
            navigator.clipboard.writeText(code).then(() => {{
                ...
                setTimeout(() => {{ btn.innerHTML = '<i class="fa-solid fa-copy"></i> Copy'; btn.classList.remove('copied'); }}, 2000);
            }});
        }}
    </script>
</body>
...
if __name__ == '__main__':
    builder = ICNBuilder()
    builder.build()#!/usr/bin/env python3
"""
ICN Blog — Static Site Builder
Reads data/modules.json and generates the complete site.
Run: python build.py
...
<body>
    ...
    <main>
        ...
        <section class="section">
            ...
            <p class="sub">Build real tools with industry-standard Python libraries</p>
            <div class="tech-grid">
                <div class="tech-item"><i class="fa-brands fa-python"></i><span>Python</span><small>Core language</small></div>
                <div class="tech-item"><i class="fa-solid fa-network-wired"></i><span>Scapy</span><small>Packet manipulation</small></div>
                <div class="tech-item"><i class="fa-solid fa-server"></i><span>Netmiko</span><small>Network automation</small></div>
                <div class="tech-item"><i class="fa-solid fa-shield"></i><span>Cryptography</span><small>Encryption</small></div>
                <div class="tech-item"><i class="fa-solid fa-brain"></i><span>Scikit-learn</span><small>Machine Learning</small></div>
                <div class="tech-item"><i class="fa-solid fa-globe"></i><span>Flask</span><small>Web dashboards</small></div>
                <div class="tech-item"><i class="fa-solid fa-cloud"></i><span>Docker</span><small>Containerization</small></div>
                <div class="tech-item"><i class="fa-solid fa-database"></i><span>SQLite</span><small>Data storage</small></div>
            </div>
        </section>
    </main>
...
<body>
    ...
    <main class="container" style="max-width:800px;margin:0 auto;padding:3rem 1.25rem;">
        <h1 style="font-size:2rem;margin-bottom:1rem;">About ICN Blog</h1>
        ...
        <div class="about-card">
            ...
            <p><a href="portfolio/index.html">Portfolio</a> &nbsp;·&nbsp; <a href="https://github.com/theOutcast" target="_blank" rel="noopener">GitHub</a></p>
        </div>
    </main>
...
<body>
    <header class="header">
        <div class="header-inner">
            ...
            <a href="../../index.html#modules" class="back-link"><i class="fa-solid fa-arrow-left"></i> All Modules</a>
        </div>
    </header>
    ...
    def _build_sitemap(self):
        urls = ['index.html', 'about.html', 'portfolio/index.html']
        for m in self.all_modules:
            urls.append(m['url'])
